...
class AutoRegReader(BaseRecognizer):
    def __init__(self,
        ...
        self.ins_lvl_refine = ins_lvl_refine
    ...
    def make_module(self, in_dim, d_model, fusion_pe_args, node_attn_cfg, node_attn_layers,
        ...
        self.use_node_attn = node_attn_cfg.pop('use_node', True)
        ...
        ins_refiner_cfg = rec_decoder_args.pop('ins_refiner_cfg', {})
        self.use_refine_at_last = ins_refiner_cfg.pop('use_refine_at_last', True)
        self.ins_lvl_refine_mask_current = ins_refiner_cfg.pop('mask_current', True)
        self.add_node_to_query = rec_decoder_args.pop('add_node_to_query', False)
        self.add_node_to_memory = rec_decoder_args.pop('add_node_to_memory', True)
        ...
        if not self.rec_only:
            ...
            if self.ins_lvl_refine:
                # self.ins_ocr_refiner = clones(DecoderLayer(**ins_refiner_cfg), 1)
                self.ins_kie_refiner = clones(DecoderLayer(**ins_refiner_cfg), 1)
    ...
    def rec_fusion(self, vis_feature, spa_feature, ins_mask=None):
        """
    ...
    def rec_pred(self, node_feature, visual_feature, gt_texts,
        ...
        Returns:
            ...
            pred_logits: (BN, L, C)
        """
        if self.use_node_attn and self.add_node_to_memory:
            # try both query type
            B, N, C = node_feature.shape
            # (B x N, H x W, C)
            global_feature = visual_feature.reshape(B * N, C, -1).permute(0, 2, 1) + node_feature.reshape(-1, C).unsqueeze(1)
        else:
        ...
        if not self.rec_only and self.ins_lvl_refine:
            ins_refine_mask = self.prep_ins_refine_mask(gt_texts[:, :, 1:], pad_val=self.ocr_dict['<PAD>'])
        else:
            ins_refine_mask = None
        # (B x N, L, C)
        ...
        if self.add_node_to_query and self.use_node_attn:
            query_seq += node_feature.reshape(-1, C).unsqueeze(1)
        return self.rec_attn(query_seq[:, :-1, :], global_feature, None, seq_mask,
                             ins_refine_mask=ins_refine_mask)
    ...
    def rec_greedy_pred(self, node_feature, visual_feature, gt_texts=None):
        ...
        if self.use_node_attn and self.add_node_to_memory:
            ...
            B, N, C = node_feature.shape
            # (B x N, H x W, C)
            global_feature = visual_feature.reshape(B * N, C, -1).permute(0, 2, 1) + node_feature.reshape(-1, C).unsqueeze(
                1)
        else:
            B, N, C, H, W = visual_feature.shape
            global_feature = visual_feature.reshape(B * N, C, -1).permute(0, 2, 1)
        ...
        for i in range(self.max_seq_len-1):
            ...
            if not self.rec_only and self.ins_lvl_refine:
                ins_refine_mask = self.prep_ins_refine_mask(ocr_input, pad_val=self.ocr_dict['<PAD>'])
            else:
                ins_refine_mask = None
            ...
            if not isinstance(gt_texts, type(None)):
                ...
                # if self.ins_lvl_refine and not self.use_refine_at_last:
                #     ocr_prob = soft_max_func(output['REC'][-1])
                # else:
            ...
            if not self.rec_only and self.query_type == 'BOTH':
                if self.ins_lvl_refine and not self.use_refine_at_last:
                    kie_prob = soft_max_func(output['KIE'][-1])
                else:
    ...
    def rec_attn(self, x, feature, src_mask, tgt_mask,
        ...
        if not self.rec_only:
            ...
            # todo: add instance-level rectify module here
            if self.ins_lvl_refine:
                # only refine once for now.
                # for idx, layer in enumerate(self.ins_ocr_refiner):
                #     assert idx == 0
                #     ocr_logit_refined = layer(ocr_logit, kie_logit, None, ins_refine_mask)
                for idx, layer in enumerate(self.ins_kie_refiner):
                    assert idx == 0
                    kie_logit_refined = layer(kie_logit, ocr_logit, None, ins_refine_mask)
                # logits['REC'].append(self.ocr_fc(self.rec_norm(ocr_logit_refined)))
                logits['REC'].append(logits['REC'][0])
                logits['KIE'].append(self.kie_fc(self.rec_norm(kie_logit_refined)))
