...
class AutoRegReaderV1(BaseRecognizer):
    def __init__(self,
                 rec_cfg,
                 loss=None,
                 use_ins_mask=False,
    ...
    def make_module(self, in_dim, d_model, fusion_pe_args, node_attn_cfg, node_attn_layers,
        ...
        self.use_layout_node_attn = node_attn_cfg.pop('use_layout_node', True)
        ...
        if self.use_layout_node_attn or self.use_context_node_attn:
            ...
            self.kie_modal_fuser_q = nn.Conv1d(in_channels=int(d_model*num_modal),
                                               out_channels=int(d_model*num_modal),
                                               kernel_size=1)
            self.kie_modal_fuser_kv = nn.Conv1d(in_channels=int(d_model * num_modal),
                                                out_channels=int(d_model * num_modal),
                                                kernel_size=1)
            self.kie_modal_fuser_out = nn.Conv1d(in_channels=int(d_model * num_modal),
                                                 out_channels=d_model,
                                                 kernel_size=1)
            self.kie_cross_modal_attn = nn.TransformerDecoderLayer(d_model=2*d_model, nhead=8, dim_feedforward=2024)
            ...
            self.kie_modal_fuser_q = None
            self.kie_modal_fuser_kv = None
            self.kie_cross_modal_attn = None
            self.kie_modal_fuser_out = None
        ...
        # self.layout_to_query = rec_decoder_args.pop('layout_to_query', False)
        # self.layout_to_memory = rec_decoder_args.pop('layout_to_memory', True)
        # self.context_to_query = rec_decoder_args.pop('context_to_query', False)
    ...
    def rec_pred(self, node_feature, visual_feature, gt_texts,
        ...
        Args:
            ...
            spa_feature: (B, N, K, C), spatial feature (optional)
    ...
    def rec_greedy_pred(self, node_feature, visual_feature, gt_texts=None,
                        ins_mask=None, spa_feature=None):
        """
        Fusion node-level feature and sequence feature to predict sequence of texts and iob-tags.
        Currently we only use node_feature and visual_feature.
        Args:
        ...
        for i in range(self.max_seq_len-1):
            ...
            output = self.rec_attn(query_seq, global_feature, None, seq_mask, layout_info=node_feature,
                                   context_info=context_attn, spa_feature=spa_feature)
            if not isinstance(gt_texts, type(None)):
    ...
    def rec_attn(self, x, feature, src_mask, tgt_mask, layout_info=None,
                 context_info=None, spa_feature=None):
        """
        ...
        if not self.rec_only:
            ...
            if self.forward_manner == 'PARALLEL':
                if self.use_layout_node_attn:
                    ...
                    # BN, L, C
                    x = self.kie_modal_fuser_q(x.permute(0, 2, 1)).permute(0, 2, 1)
                    #
                    B, N, K, C = spa_feature.shape
                    layout_memory = torch.cat([spa_feature, layout_info.unsqueeze(2).expand(spa_feature.shape)], dim=3).reshape(B*N, K, C)
                    layout_memory = self.kie_modal_fuser_kv(layout_memory.permute(0, 2, 1)).permute(0, 2, 1)
                    layout_attn = self.kie_cross_modal_attn(tgt=x.permute(1, 0, 2),
                                                            memory=layout_memory.permute(1, 0, 2)).permute(1, 0, 2)
                    x = self.kie_modal_fuser_out(torch.cat([x, layout_attn], dim=2).permute(0, 2, 1)).permute(0, 2, 1)
    ...
    def extract_feat(self, imgs):
        super(AutoRegReaderV1, self).extract_feat(imgs)
