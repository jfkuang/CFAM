...
class AutoRegReaderCMInside(BaseRecognizer):
    def __init__(self,
                 rec_cfg,
                 loss=None,
                 use_ins_mask=False,
...
m
    ...
    def rec_greedy_pred(self, node_feature, visual_feature, gt_texts=None,
        ...
        for i in range(self.max_seq_len-1):
            ...
            output = self.rec_attn(query_seq, global_feature, None, seq_mask, layout_info=node_feature,
                                   context_info=None, sorted_idx=sorted_idx, num_ins=N, ins_mask=ins_mask)
            if not isinstance(gt_texts, type(None)):
    ...
    def rec_attn(self, x, feature, src_mask, tgt_mask, layout_info=None,
                 ...
                 sorted_idx=None, num_ins=-1, ins_mask=None):
        """
        ...
        if not self.rec_only:
            ...
            if self.forward_manner == 'PARALLEL':
                if self.use_context_node_attn and self.context_to_kie:
                    BN, L, C = x.shape
                    assert BN % num_ins == 0, f"Unmatched: B*N={BN}, N={num_ins}"
                    B = BN // num_ins
                    query_memory = x.unsqueeze(1).expand(BN, L, L, C)
                    # (B x N, L', C), seq_mask with shape (B x N, 1, L, L)
                    query_memory = query_memory.masked_fill(tgt_mask.squeeze(1).unsqueeze(-1) == 0, 0).sum(2).div(
                        tgt_mask.squeeze(1).sum(2).unsqueeze(-1) + 1e-6)

                    # (B x L', N, C)
                    query_memory = query_memory.reshape(B, num_ins, L, C).permute(0, 2, 1, 3).reshape(B * L, num_ins, C)
                    if self.sort_context:
                        # convert to B*L' lists
                        sort_ = []
                    ...
                    if self.node_attn_type == 'origin':
                        context_info = self.rec_fusion_attn(query_memory.permute(1, 0, 2),
                                                            src_key_padding_mask=ins_mask_).permute(1, 0, 2)
                    else:
                        context_info = self.rec_fusion_attn(query_memory,
                                                            attention_mask=ins_mask_.unsqueeze(1).unsqueeze(2) * -1e6,
                                                            head_mask=[None] * self.node_attn_layers)[0]
                    context_info = context_info.reshape(B, L, num_ins, C).permute(0, 2, 1, 3).reshape(-1, L, C)
    ...
    def extract_feat(self, imgs):
        super(AutoRegReaderCMInside, self).extract_feat(imgs)
