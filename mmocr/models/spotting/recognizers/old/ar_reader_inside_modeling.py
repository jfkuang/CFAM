...
from mmocr.models.spotting.modules import build_text_encoder, build_text_decoder, KIEDecoder, GlobalModeling
...
class AutoRegReaderIM(BaseRecognizer):
    def __init__(self,
                 rec_cfg,
                 loss=None,
                 use_ins_mask=False,
    ...
    def make_module(self, text_encoder_args, d_model, fusion_pe_args, node_attn_cfg,
        ...
        if self.use_layout_node_attn or self.use_context_node_attn:
            if self.modeling_inside:
                # TODO: CHECKPOINT
                self.gm = GlobalModeling(self.use_layout_node_attn, self.use_context_node_attn)
                ...
                self.rec_fusion_attn = build_node_attn(self.node_attn_type, d_model,
                                                       node_attn_layers,
                                                       node_attn_cfg.pop('attn_args', {}))
        ...
        if not self.rec_only:
            ...
            if self.use_context_node_attn and self.use_context_last:
                num_modal += 1
            if num_modal > 1 and not self.modeling_inside:
    ...
    def get_global_modeling_status(self, node_attn_cfg):
        ...
        # whether to build context outside, and perform context & layout merging outside
        self.modeling_inside = node_attn_cfg.pop('modeling_inside', True)
        # whether to build context with shared transformer's output
        self.context_inside = node_attn_cfg.pop('context_inside')
    ...
    def forward_train(self, vis_feature=None, spa_feature=None, labels=None,
        ...
        if self.use_layout_node_attn:
            ...
            nodes = self.layout_nodes_modeling(vis_feature,
                                               spa_feature,
    ...
    def layout_nodes_modeling(self, vis_feature, spa_feature, ins_mask=None, sorted_idx=None):
        """
    ...
    def context_nodes_modeling(self, x, seq_mask, ins_mask, shapes, sorted_idx=None):
        """
        Context nodes modeling.
        Args:
            x: Tensor, (B x N, L, C)
            shapes: tuple, (B, N, L, C)

        Returns:

        """
        if not self.use_context_node_attn:
            return None
        B, N, L, C = shapes
        # from (B x N, L, C) broadcast to (B x N, L, L, C)
        query_memory = x.unsqueeze(1).expand(B * N, L, L, C)
        ...
        # (B x L, N, C)
        query_memory = query_memory.reshape(B, N, L, C).permute(0, 2, 1, 3).reshape(B * L, N, C)
        ...
        ins_mask_ = ins_mask.unsqueeze(1).expand(B, L, N).reshape(-1, N)
        if self.node_attn_type == 'origin':
            ...
            context_attn = self.rec_fusion_attn(query_memory,
                                                ...
                                                head_mask=[None] * self.node_attn_layers)[0]
        return context_attn.reshape(B, L, N, C).permute(0, 2, 1, 3).reshape(-1, L, C)
...
m
        ...
        if not self.modeling_inside:
            if not self.context_inside:
                context_attn = self.context_nodes_modeling(query_seq[:, :-1, :], seq_mask,
                                                           ins_mask, (B, N, L-1, C),
                                                           sorted_idx=sorted_idx)
            else:
                context_attn = None
    ...
    def rec_greedy_pred(self, node_feature, visual_feature, gt_texts=None,
        ...
        for i in range(self.max_seq_len-1):
            ...
            if not self.modeling_inside:
                if not self.context_inside:
                    context_attn = self.context_nodes_modeling(query_seq, seq_mask,
                                                               ins_mask, (B, N, cur_l, C),
                                                               sorted_idx=sorted_idx)
                else:
                    context_attn = None
            else:
                context_attn = None
            output = self.rec_attn(query_seq, global_feature, None,
                                   seq_mask, layout_info=node_feature,
    ...
    def rec_attn(self, x, feature, src_mask, tgt_mask, layout_info=None,
                 ...
                 sorted_idx=None, ins_mask=None, shapes=None):
        """
        ...
        if not self.rec_only:
            ...
            if self.forward_manner == 'PARALLEL':
                if (self.use_layout_node_attn and self.layout_to_kie) or (self.use_context_node_attn and self.context_to_kie):
                    if not self.modeling_inside:
                        ...
                        if self.context_inside:
                            context_info = self.context_nodes_modeling(x, tgt_mask, ins_mask, shapes,
                                                                       sorted_idx=sorted_idx)
            ...
            if (self.use_layout_node_attn and self.use_layout_last) or (self.use_context_node_attn and self.use_context_last):
                if not self.modeling_inside:
                    ...
                    raise RuntimeError("When modeling global information during sequence decoding" +
                                       "it no longer needs to be added multi-modal feature at last.")
            else:
    ...
    def print_global_modeling_cfg(self):
        ...
        global_modeling_cfg += f"modeling_inside: {self.modeling_inside}\n"
        global_modeling_cfg += f"context_inside: {self.context_inside}\n"
    ...
    def extract_feat(self, imgs):
        super(AutoRegReaderIM, self).extract_feat(imgs)
