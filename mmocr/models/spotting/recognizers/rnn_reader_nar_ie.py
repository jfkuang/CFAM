...
class RNNReaderNARIE(BaseRecognizer):
    def __init__(self,
                 rec_cfg,
                 loss=None,
                 use_ins_mask=False,
    ...
    def make_module(self, text_encoder_args, d_model, fusion_pe_args, node_attn_cfg,
        ...
        if not self.rec_only:
            ...
            if self.forward_manner == 'SEQUENCE' and not keep_kie_transformer:
                ...
                self.use_ext_rec = crf_args.pop('use_ext_rec', False)
                if self.use_ext_rec:
                    self.ext_ocr_fc = nn.Linear(d_model, len(self.ocr_dict))
...
m
        ...
        if self.rec_only:
            ...
            if self.forward_manner == 'PARALLEL' or self.keep_kie_transformer:
                if self.use_kv_catcher and (self.training or last_iter):
                    ...
                    kie_cls_res = self.kie_fc(self.kie_norm(kie_logit))
                if self.use_ext_rec:
                    logits['REC'].append(self.ext_ocr_fc(self.rec_norm(kie_logit)))
            else:
                kie_cls_res = None
            ...
            if len(output['CRF']) > 0:
                ...
                if self.use_crf:
                    for tags in output['CRF']:
                        ...
                        if self.keep_kie_res:
                            if self.use_ext_rec:
                                # [origin_rec, origin_rec, new_rec]
                                results['REC']['indexes'].insert(0, results['REC']['indexes'][0])
                                results['REC']['scores'].insert(0, results['REC']['scores'][0])
                                # [origin_kie, new_kie]
    ...
    def extract_feat(self, imgs):
        super(RNNReaderNARIE, self).extract_feat(imgs)
