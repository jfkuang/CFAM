...
'''
@Project ：ie_e2e 
@File    ：ar_reader_nar_ie_0726_kvc_decoder.py
@IDE     ：PyCharm 
@Author  ：jfkuang
@Date    ：2022/10/7 10:53 
'''
...
class AutoRegReaderNARIE0726_kvc_decoder(BaseRecognizer):
    def __init__(self,
                 rec_cfg,
                 loss=None,
                 use_ins_mask=False,
...
m
    ...
    def rec_attn(self, x, feature, src_mask, tgt_mask, layout_info=None,
        ...
        if self.rec_only:
                        ...
                        if self.global_dec:
                            ...
                            if idx == 0:
                                ...
                                if self.nar_kie_mask != 'NONE':
                                    ...
                                    kie_logit = layer(kie_logit, feature, src_mask, tgt_mask)
            if self.use_kv_catcher and self.training:
                ...
                if self.kvc_feat_type == 'K':
                    ...
                    kvc_losses = self.kv_catcher(feature, x,
                                                 shape, kvc_mask,
                                                 self.kvc_context_encoder, self.kvc_texture_encoder, self.kvc_context_embedding, self.kvc_texture_embedding)
                    logits.update(kvc_losses)
            ...
            if self.forward_manner == 'PARALLEL' or self.keep_kie_transformer:
                if self.use_kv_catcher and self.kv_catcher_type != 'OneStageContrast' and (self.training or last_iter):
                    B = x.shape[0] // num_ins
                    L = x.shape[1]
                    C = x.shape[-1]
    ...
    def prep_kvc_gt(self, entities, num_boxes, gt_texts=None, img_metas=None):
        ...
        if self.kv_catcher_type == 'OneStageCatcher' or self.kv_catcher_type == 'CustomOneStageCatcher' or self.kv_catcher_type=='OneStageContrast':
            if self.training:
                B, N, L = entities.shape
    ...
    def extract_feat(self, imgs):
        super(AutoRegReaderNARIE0726_kvc_decoder, self).extract_feat(imgs)
