...
#total methods: transform decoder-->rnn+attention
@DETECTORS.register_module()
class RNNRecNARIE(BaseRecognizer):
    def __init__(self,
                 rec_cfg,
                 loss=None,
                 use_ins_mask=False,
    ...
    def make_module(self, text_encoder_args, d_model, fusion_pe_args, node_attn_cfg,
                    ...
                    rec_layer_num, max_seq_len, crf_args, rec_dual_layer_num=0,
        ...
        # prepare transformer layer for recognition and extraction-->rnn+attention
        decoder_type = rec_decoder_args.pop('type', 'rnn')
        ...
        if not self.rec_only:
            ...
            if self.forward_manner == 'SEQUENCE':
                self.rec_layer_extend = build_text_decoder(type=decoder_type, num_block=1, **rec_decoder_args)
                ...
                if kv_catcher_args:
                    ...
                    self.kvc_feat_type = kv_catcher_args.pop('kvc_feat_type', 'entity_instance')
                    ...
                    # kv_catcher_args.update(entity_feature=self.entity_feature)
                    # kv_catcher_args.update(instance_feature=self.instance_feature)
...
m
    ...
    def rec_attn(self, x, feature, src_mask, tgt_mask, layout_info=None,
        ...
        # ocr_logit = None
        # for idx, layer in enumerate(self.rec_layer):
        #     if idx == 0:
        ...
        #                                            sorted_idx=sorted_idx)
            ...
            if self.forward_manner == 'PARALLEL':
                ...
                if not self.keep_kie_transformer:
                    for idx, layer in enumerate(self.rec_layer_extend):
                        ocr_logit = layer(ocr_logit, feature, src_mask, tgt_mask)
                else:
                    kie_logit = None
                    for idx, layer in enumerate(self.rec_kie_layer):
    ...
    def extract_feat(self, imgs):
        super(RNNRecNARIE, self).extract_feat(imgs)
