...
# todo: finish and verify this code ~ DDL 6.4
...
class CustomTRIE(BaseRecognizer):
    ...
    def make_module(self, text_encoder_args, d_model, fusion_pe_args,
                    ...
                    crf_args, rec_dual_layer_num=1, gt_mode=False):
        ...
        # modules for information extraction
        self.infor_context_module = build_connect(infor_context_module)
        self.disentangle = crf_args.pop('disentangle', False)
        if self.disentangle:
            self.use_kie_loss = crf_args.pop('use_kie_loss', True)
            self.keep_kie_res = crf_args.pop('keep_kie_res', True)
            self.rec_kie_layer = build_text_decoder(type=decoder_type, num_block=1, **rec_decoder_args)
    ...
    def forward_train(self, vis_feature=None, labels=None,
        ...
        Returns:
            losses: dict
        """
        # todo: debug, the GT results seems way to low, maybe related to the input format
...
m
        ...
        if self.gt_mode:
            L = gt_texts.shape[-1]
            ocr_logit = self.ocr_embedding(gt_texts.reshape(-1, L)[:, 1:])
            return self.rec_attn_gt(ocr_logit, visual_feature, det_boxes, img_metas,
                                    gt_texts, gt_entities, sorted_idx=sorted_idx)
    ...
    def rec_gt_pred(self, visual_feature, gt_texts, gt_boxes,
                    img_metas, sorted_idx=None):
        """
        Inference with ground-truth OCR
        Args:
            visual_feature: Tensor with shape (B, N, C, H, W)
            gt_texts: (B, N, L)
            gt_boxes:
            img_metas:
            sorted_idx:
        ...
        ocr_logit = self.ocr_embedding(gt_texts.reshape(-1, L)[:, 1:])
        output = self.rec_attn_gt(ocr_logit, visual_feature, gt_boxes, img_metas,
                                  gt_texts, sorted_idx=sorted_idx)
    ...
    def rec_attn_gt(self, ocr_logit, vis_feature, det_boxes, img_metas,
                    gt_texts, gt_entities=None, sorted_idx=None):
        """
        branch for KIE
        Args:
            ocr_logit: Tensor with shape (BN, L, C)
            vis_feature:
            det_boxes:
            img_metas:
            gt_texts:
    ...
    def rec_attn(self, x, feature, src_mask, tgt_mask,
        ...
        if self.disentangle:
            ...
            if self.use_kie_loss:
                logits['KIE'].append(self.kie_fc(self.kie_norm(kie_logit)))
            # multimodal context module
            # with shape (B, N, C)
            multimodal_context = self.infor_context_module([vis_feature, kie_logit],
                                                           boxes=det_boxes,
                                                           img_metas=img_metas)

            self.kie_decoder(ocr_logits=kie_logit,
                             multi_modal_context=multimodal_context,
                             texts=gt_texts[:, :, 1:] if self.training else None,
                             tags=gt_entities[:, :, 1:] if self.training else None,
                             logits_logger=logits,
    ...
    def simple_test(self, vis_feature=None, num_boxes=None, gt_texts=None,
        ...
        if self.gt_mode:
            output = self.rec_gt_pred(vis_feature, gt_texts, det_boxes,
                                      img_metas, sorted_idx=sorted_idx)
            results['REC']['indexes'].append(gt_texts[:, :, 1:])
            results['REC']['scores'].append(torch.ones(gt_texts.shape, device=vis_feature.device))
            if self.use_crf:
                ...
                raise RuntimeError("TRIE is required to use CRF")
            return results
        ...
        output = self.rec_greedy_pred(vis_feature, gt_texts=gt_texts,
                                      ...
                                      det_boxes=det_boxes, img_metas=img_metas)
        soft_max_func = nn.Softmax(dim=2)
        if self.disentangle and self.keep_kie_res:
            key_list = ['REC', 'KIE']
        else:
            key_list = ['REC']
        for key in key_list:
