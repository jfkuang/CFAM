...
'''
@Project ：ie_e2e 
@File    ：ar_reader_nar_ie_0726_kvc.py
@IDE     ：PyCharm 
@Author  ：jfkuang
@Date    ：2022/9/22 21:41 
'''
...
class AutoRegReaderNARIE0726_kvc(BaseRecognizer):
    def __init__(self,
                 rec_cfg,
                 loss=None,
                 use_ins_mask=False,
    ...
    def make_module(self, text_encoder_args, d_model, fusion_pe_args, node_attn_cfg,
        ...
        if not self.rec_only:
            ...
            if self.forward_manner == 'SEQUENCE' and not keep_kie_transformer:
                self.rec_ocr_layer_extend = build_text_decoder(type=decoder_type, num_block=rec_dual_layer_num, **rec_decoder_args)
                ...
                if kv_catcher_args:
                    ...
                    self.kvc_feat_type = kv_catcher_args.pop('kvc_feat_type', 'E')
                    self.kvc_rec_embed = kv_catcher_args.get('use_rec_embed', False)

                    #the type of encoder and embedding in kvc
                    self.kvc_context_encoder = kv_catcher_args.pop('use_context_encoder', False)
                    self.kvc_texture_encoder = kv_catcher_args.pop('use_texture_encoder', False)
                    self.kvc_context_embedding = kv_catcher_args.pop('use_context_embedding', False)
                    self.kvc_texture_embedding = kv_catcher_args.pop('use_texture_embedding', False)
                    self.add_decoder = kv_catcher_args.pop('add_decoder', False)

                    #kvc type:none,matrix,self_attention,cross_attention,clip
                    self.kvc_type = kv_catcher_args.get('kvc_type', 'matrix')
                    self.kvc_weights = kv_catcher_args.get('kvc_weights', 1.0)
...
m
        ...
        return self.rec_attn(query_seq[:, :-1, :], global_feature, None, seq_mask,
                             ...
                             kvc_mask=kvc_mask, shape=visual_feature.shape)
    ...
    def rec_greedy_pred(self, node_feature, visual_feature, gt_texts=None,
        ...
        for i in range(self.max_seq_len-1):
            ...
            if self.global_dec:
                ...
                global_seq_mask = None
                if self.use_kv_catcher and self.kv_catcher_type != 'OneStageContrast':
                    pad_mask = self.prep_pad_mask(ocr_input, pad_val=self.ocr_dict['<PAD>'])
                else:
                    pad_mask = None
            ...
            output = self.rec_attn(query_seq, global_feature, None, seq_mask, layout_info=node_feature,
                                   ...
                                   last_iter=i == (self.max_seq_len-2), shape=visual_feature.shape)
            if not isinstance(gt_texts, type(None)):
    ...
    def rec_attn(self, x, feature, src_mask, tgt_mask, layout_info=None,
                 ...
                 global_seq_mask=None, kvc_mask=None,
                 last_iter=False, shape=None):
        """
        Transformer for recognition
        Args:
            x(texture feature): (B x N, L, C)
            feature(context feature): (B*N,H*W,C)
            src_mask:
            tgt_mask:
        ...
        if self.rec_only:
            ...
            if self.use_kv_catcher and (self.training or last_iter):
                #ocr_logit:(B*N,L,C) kie_logit:(B*N,L,C)
                ...
                if self.kvc_feat_type == 'K':
                    ...
                    if self.training and self.kvc_type != 'none':
                        ...
                        entity_logits = self.kv_catcher(feature, x, seq_mask,
                                                        ...
                                                        self.kvc_texture_embedding)
                elif self.kvc_feat_type == 'D':
                    kvc_losses, entity_logits = self.kv_catcher(ocr_feature, kie_feature,
                                                                shape, kvc_mask,
                                                                )
                    logits.update(kvc_losses)
                elif self.kvc_feat_type == 'L':
                    kvc_losses, entity_logits = self.kv_catcher(kie_logit, ocr_logit,
                                                                shape, kvc_mask,
                                                                )
                    logits.update(kvc_losses)
                else:
            ...
            if self.forward_manner == 'PARALLEL' or self.keep_kie_transformer:
                if self.use_kv_catcher and (self.training or last_iter) and self.add_decoder:
                    B = x.shape[0] // num_ins
                    L = x.shape[1]
                    C = x.shape[-1]
                    N = num_ins
                    ...
                    elif 'OneStageContrast' in str(type(self.kv_catcher)) and self.kvc_feat_type == 'E':
                        if self.kvc_type == 'none':
                            # 这里的entity_logits就是前面的特征：(B,N,C) , kie_logit:(BN,L,C) -> (BN,L,C)
                            # ipdb.set_trace()
                            entity_logits = entity_logits.unsqueeze(3).expand(B, N, C, L).transpose(-1, -2)
                            entity_logits = entity_logits.reshape(-1, L, C)
                            kie_logit += entity_logits
                            kie_cls_res = self.kie_fc(self.kie_norm(kie_logit))

                        elif self.kvc_type == 'self_attention':
                            #entity_logits:(B,N,N)->(B,N)->(B,N,L)->(B,N,L,C)->(BN,L,C)
                            entity_logits = torch.mean(entity_logits, dim=2)
                            entity_logits = entity_logits.unsqueeze(2).expand(B, N, L).unsqueeze(3).expand(B, N, L, C)
                            entity_logits = entity_logits.reshape(-1, L, C)
                            kie_logit += entity_logits
                            kie_cls_res = self.kie_fc(self.kie_norm(kie_logit))

                        elif self.kvc_type == 'cross_attention2':
                            #entity_logits:(B,N,C)->(BN,C)
                            entity_logits = entity_logits.reshape(-1, C)
                            # ipdb.set_trace()
                            kie_logit += entity_logits.unsqueeze(1)
                            kie_cls_res = self.kie_fc(self.kie_norm(kie_logit))
                            ...
                            # B, num_cls, N -> B, num_cls, 1, N
                            entity_logits = entity_logits.unsqueeze(2)
                            # BN, L, C -> B, CL, N -> B, 1, CL, N
                            kie_logit = kie_logit.reshape(B, -1, N).unsqueeze(1)
                            # (B,1,CL,N) *(B,cls,1,N)-> B, num_cls, CL, N -> B, NL, num_cls, C -> B, NL, num_cls * C
                    ...
                    elif 'OneStageContrast' in str(type(self.kv_catcher)) and self.kvc_feat_type == 'D':
                    ...
                    elif 'LogitContrast' in str(type(self.kv_catcher)) and self.kvc_feat_type == 'L':
                        # B, num_cls, C -> B, num_cls, 1, C
                        entity_logits = entity_logits.unsqueeze(2)
                        # BN, L, C -> B, NL, C -> B, 1, NL, C
                        kie_logit = kie_logit.reshape(B, -1, C).unsqueeze(1)
                ...
                kie_cls_res = None
            #经过contrast过程的instance feature (B*N,H*W,C)
    ...
    def extract_feat(self, imgs):
        super(AutoRegReaderNARIE0726_kvc, self).extract_feat(imgs)
