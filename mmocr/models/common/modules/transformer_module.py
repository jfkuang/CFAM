...
class PositionalEncoding(nn.Module):
    ...
    def forward(self, x, sorted_idx=None):
        """
        Args:
            x (Tensor): Tensor of shape (batch_size, pos_len, d_hid, ...)
            sorted_idx (list of lists): within each list, it determines the
                sorted index with respect to box position
        """
        self.device = x.device
        if sorted_idx:
            for i in range(x.shape[0]):
                sort_ = [sorted_idx[i].index(x) for x in range(len(sorted_idx[i]))]
                sort_ = sort_ + [_ for _ in range(len(sorted_idx[i]), x.shape[1])]
                x[i, :, :] += self.position_table[0, sort_].clone().detach()
        else:
