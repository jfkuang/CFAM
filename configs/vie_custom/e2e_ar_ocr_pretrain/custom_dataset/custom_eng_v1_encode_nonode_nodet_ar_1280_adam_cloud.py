...
_base_ = [
    '../../_base_/ocr_datasets/custom_eng_v1_ar_cloud.py',
    '../../_base_/e2e_pipelines/ocr_pretrain_pipeline_1280.py'
]

det_module = det.copy()
det_module['type'] = 'ReI_OCRMaskRCNN'
det_module['roi_head']['type'] = 'ReI_StandardRoIHead'
# remove det head to force the model to train REC module
det_module['rpn_head'] = None
det_module['roi_head'] = None
...
model = dict(
    ...
    neck=dict(
        feature_fuser=dict(
            feat_num_in=4, in_channels=256, out_channels=128, sum_up=False,
        ),
        roi_extractor=dict(
        ...
        spatial_embedding=dict(
            ...
            fuse_method='sum',
            pos_input_attn_cfg=dict(
                nhead=8
            ),
        ),
        seq_memory_encoder=dict(
            input_dim=512,
            gcb_config=dict(
                ratio=0.0625,
                headers=1,
    ...
    vis_cfg=dict(
        ...
        show_text=True,
        show_entity=True,
        dict_file='/apdcephfs/share_887471/interns/v_willwhua/dataset/ie_e2e/custom_eng_v1/merged/dict.json',
        class_file=None,
        auto_reg=True
    ),
    pretrain=None,
