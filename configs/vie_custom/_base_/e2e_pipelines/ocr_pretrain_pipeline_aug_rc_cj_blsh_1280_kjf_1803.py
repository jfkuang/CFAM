...
'''
@Project ：ie_e2e 
@File    ：ocr_pretrain_pipeline_aug_rc_rr_cj_blsh_1280_kjf.py
@IDE     ：PyCharm 
@Author  ：jfkuang
@Date    ：2022/6/4 17:22 
'''
...
train_pipeline = [
    ...
    #add blur
    dict(type='DataAug',
         args=[dict(cls='MedianBlur', k=[1, 3]), dict(cls='GaussianBlur', sigma=[0.0, 1.0]), dict(cls='AverageBlur', k=[1, 3])]),
    #add sharpen
    dict(type='DataAug',
         args=[dict(cls='Sharpen', lightness=[0.75, 1.5])]),
